...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        url="https://github.com/cofedish/3x-UI-agents/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    if [[ $release == "alpine" ]]; then
        wget --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/cofedish/3x-UI-agents/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
