...
update_x-ui() {
	...
	tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/cofedish/3xui-agents/releases/latest" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
	if [[ ! -n "$tag_version" ]]; then
		echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
		tag_version=$(${curl_bin} -4 -Ls "https://api.github.com/repos/cofedish/3xui-agents/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
		if [[ ! -n "$tag_version" ]]; then
			_fail "ERROR: Failed to fetch x-ui version, it may be due to GitHub API restrictions, please try it later"
		fi
	fi
	echo -e "Got x-ui latest version: ${tag_version}, beginning the installation..."
	${wget_bin} -N -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/cofedish/3xui-agents/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2>/dev/null
	if [[ $? -ne 0 ]]; then
		echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
		${wget_bin} --inet4-only -N -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/cofedish/3xui-agents/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2>/dev/null
		if [[ $? -ne 0 ]]; then
	...
	${wget_bin} -O /usr/bin/x-ui https://raw.githubusercontent.com/cofedish/3xui-agents/main/x-ui.sh >/dev/null 2>&1
	if [[ $? -ne 0 ]]; then
		echo -e "${yellow}Trying to fetch x-ui with IPv4...${plain}"
		${wget_bin} --inet4-only -O /usr/bin/x-ui https://raw.githubusercontent.com/cofedish/3xui-agents/main/x-ui.sh >/dev/null 2>&1
		if [[ $? -ne 0 ]]; then
	...
	if [[ $release == "alpine" ]]; then
		...
		${wget_bin} -O /etc/init.d/x-ui https://raw.githubusercontent.com/cofedish/3xui-agents/main/x-ui.rc >/dev/null 2>&1
		if [[ $? -ne 0 ]]; then
			${wget_bin} --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/cofedish/3xui-agents/main/x-ui.rc >/dev/null 2>&1
			if [[ $? -ne 0 ]]; then
